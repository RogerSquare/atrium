...
github_pr_url: https://github.com/your-org/your-repo/pull/42   # optional; fills in PR badge + review state even if the branch is deleted locally
---
```
...
## Reusable-knowledge documentation (optional, project-specific):

If your team keeps a shared knowledge base — a wiki, a docs site, an ADR folder —
document a task's reusable output there before moving it to `review`, and add a
`- **Wiki:** ...` (or equivalent) bullet to the task's Comments recording where.

The specifics — where that knowledge base lives, how it deploys, exactly when an
entry is required — are house rules, not part of the shipped protocol. Put them in
an untracked `CLAUDE.local.md` at the repo root: the backend appends that file to
these instructions for spawned agents whenever it exists (see
devops-agent-contract-001). A template lives at the end of this file's history in
git if you want a worked example.

Atrium-internal work that is tightly coupled to this repo's task/kanban model does
NOT need an external entry — note why in the task's Comments instead.
